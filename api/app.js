...
async function getQNA(s) {
    const arr = s.split('<|qstart|>')
    // console.log(arr)
    let QNA = []
    for (let i = 1; i < arr.length; i+=2){
        // console.log(arr[i].trim())
        let QA = arr[i].split('A: ')
        // console.log(QA)
        // console.log(QA[0].substring(3))
        if(QA.length>=2){
          const Q = QA[0].substring(3).trim()
          const A = QA[1].trim()
          console.log(Q)
          console.log(A)
          let pog = [Q,A]
          QNA.push({question:Q,ans:A})
          // console.log(pog.toString())
        }
    }
    // console.log("asdsadas"+QNA.toString())
    return QNA
}
...
app.post('/getQuestions', async (req, res) =>{
    ...
    console.log(apiBody)
    var k = await getQNA(r.choices[0].text)
    // getQNA(r.choices[0].text)
    console.log(k)
